...
int main(void) // 
{
	...
	initCmd_Handler();
	initMsg_Handler();
	char msg_init[] = "\n\rOperation Modes For Input GPIO:\n\r1) Voltage Reader\n\r2) High Voltages Alert\n\r3) Low Voltages Alert\n\r4) Timed-Scheduled Voltage Reader\n\rEnter Command:\n\r";
	int i = 0;
	for(i = 0; i<=CHARCOUNT(msg_init);i++){
		bUSART1_putchar(msg_init[i]);
	}
/* test
...
*/	
	...
	//Voltage_install();
	//timed_install(000000010000000001);
	while (1)
	{

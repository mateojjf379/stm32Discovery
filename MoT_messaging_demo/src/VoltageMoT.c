...
char voltage_remove_msg[]="Removing Voltage reader\n\r";
char voltage_done_msg[]="Done reading samples. Removing Device ...\n\r";
char volt_badrequest_msg[]="Bad request -- volt command not recognized\n\r";

void Voltage_reading(void);
void voltage_remove(void);
int ADC_check();
int counter = 0;
...
void voltage_remove()	//turn off RedLED, remove RedLED task
{	//
	TIM2_disable();
	resetRedLED();
	unlink_Ctask(&myCTL);
	post_Cdevicemsg(&myCTL,voltage_remove_msg,CHARCOUNT(voltage_remove_msg)); //done AFTER task is unlinked ??TODO figure out why
}
...
void Voltage(void *Cmdtail)	//dispatched from Devicetable by Cmd_handler(). Enter with r0 pointing to the byte following the device code
{
	switch(*((uint8_t *)Cmdtail)) {
		...
		case 2:	voltage_remove(); break; 		// remove RedLED from the task list

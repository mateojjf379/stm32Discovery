...
// ------ Red_LED messages
char install_msg[]= "Installing RedLED\n\r";
...
void RedLED_on(void) //checks 'on' duration, may switch task to RedLED_off
{
	setRedLED();	//turn on LED TODO where?
	int check = 0;
	
	if( RedLEDcontrol.runflag == 0 ) { //we are not blinking so exit
		; //TODO
	}
	else { //we'll blink
		check = TIM2_check();
		...
		if( check == 0 ) { //its not time to blink so exit
			; //TODO
		}
...
void RedLED_off(void) //checks 'off' duration, may switch task to RedLED_on
{
...
void RedLED_install(void * Cmdtail) //initial installation
{	//usually hardware is initialized first, but was already initialized by call in main() to initPG13PG14()
	...
	//RedLEDcontrol.count= RedLEDcontrol.oncount;
	link_Ctask(&myCTL,RedLED_on);	
	//send info message to PC
	post_Cdevicemsg(&myCTL,install_msg,CHARCOUNT(install_msg));
	return;	// to Cmd_Handler()
}

void RedLED_update(void * Cmdtail)	//change oncount and offcount 
{	//Cmdtail will contain uint8_t Device, uint8_t Cmd, uint32_t ontime, uint32_t offtime in a hex ASCII string
	//uint32_t *args = (uint32_t *)Cmdtail; //msg: RedLED|update|oncount|offcount
	//RedLEDcontrol.oncount= *args++;
	//RedLEDcontrol.offcount=  *args;
	post_Cdevicemsg(&myCTL,update_msg,CHARCOUNT(update_msg));
	return;	// to Cmd_Handler()
}

void RedLED_pauseresume(void *Cmdtail)	// set/reset runflag 
{	//Cmdtail will contain uint8_t Device, uint8_t Cmd, uint8_t Control in a hex ASCII string
	RedLEDcontrol.runflag= *((uint8_t *)Cmdtail);
	if(RedLEDcontrol.runflag==0) 
		post_Cdevicemsg(&myCTL,pause_msg,CHARCOUNT(pause_msg));
	else
		post_Cdevicemsg(&myCTL,resume_msg,CHARCOUNT(resume_msg));
	return;	// to Cmd_Handler()
	
}

void RedLED_remove()	//turn off RedLED, remove RedLED task
{	//
	TIM2_disable();
	resetRedLED();
	unlink_Ctask(&myCTL);
	post_Cdevicemsg(&myCTL,remove_msg,CHARCOUNT(remove_msg)); //done AFTER task is unlinked ??TODO figure out why
}

//------ RedLED command dispatcher		

void RedLED(void *Cmdtail)	//dispatched from Devicetable by Cmd_handler(). Enter with r0 pointing to the byte following the device code
{
	switch(*((uint8_t *)Cmdtail)) {
		case 0:	RedLED_install(Cmdtail+1); break; 		// hardware initialization and install blink task
